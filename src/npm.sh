...
_patch_table() {
    table="$(_patch_util_edit_table_option '--workspace;[`_choice_workspace`]')"
    if [[ "$*" == "npm config "* ]]; then
        echo "$table" | _patch_util_edit_table_argument 'key;[`_choice_config_key`]' 'key-value;[`_choice_config_key`]'
    elif [[ "$*" == "npm run-script" ]]; then
