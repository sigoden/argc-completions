...
# @flag -4                           (use IPv4 query transport only)
# @flag -6                           (use IPv6 query transport only)
# @option -b <address[#port]>        (bind to source address/port)
# @option -c[`_choice_c`] <class>    (specify query class)
# @option -f <filename>              (batch mode)
# @option -k <keyfile>               (specify tsig key file)
# @flag -m                           (enable memory usage debugging)
# @option -p <port>                  (specify port number)
# @option -q <name>                  (specify query name)
# @flag -r                           (do not read ~/.digrc)
# @option -t <type>                  (specify query type)
# @flag -u                           (display times in usec instead of msec)
# @option -x <dot-notation>          (shortcut for reverse lookups)
# @option -y <[hmac:]name:key>       (specify named base64 tsig key)
