...
# @option --cli-config-file <path>         Path to a JSON file containing CLI options.
...
# @option --out-file-extension <string>    Use a specific extension for the output files [default: js] (default: "js")
# @flag -D --copy-files                    When compiling a directory copy over non-compilable files
# @flag --strip-leading-paths              Remove the leading directory (including all parent relative paths) when building the final output path (default: false)
...
# @option --workers <number>               The number of workers to use for parallel processing
