...
# @option -g --group[`_choice_group`] <group>    specify the primary group
# @option -G --supp-group[`_choice_group`] <group>  specify a supplemental group
...
# @option -s --shell[`_choice_shell`] <shell>    run <shell> if /etc/shells allows it
