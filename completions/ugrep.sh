...
# @option -C --context <NUM>                       Print NUM lines of leading and trailing context surrounding each match.
...
# @flag --confirm                                  Confirm actions in -Q query mode.
...
# @option -g --glob <GLOBS>                        Search only files whose name matches the specified comma-separated list of GLOBS, same as --include='glob' for each `glob' in GLOBS.
...
# @option -K --range <FIRST[,LAST]>                Start searching at line FIRST, stop at line LAST when specified.
...
# @flag --lines                                    Apply Boolean queries to match lines, the opposite of --files.
# @option -M --file-magic <MAGIC>                  Only files matching the signature pattern MAGIC are searched.
# @option -m --max-count <NUM>                     Stop reading the input after NUM matches in each input file.
...
# @option -N --neg-regexp <PATTERN>                Specify a negative PATTERN used during the search of the input: an input line is selected only if it matches any of the specified patterns unless a subpattern of PATTERN.
...
# @flag -o --only-matching                         Print only the matching part of lines.
# @flag --only-line-number                         The line number of the matching line in the file is output without displaying the match.
# @flag --files                                    Apply Boolean queries to match files, the opposite of --lines.
# @flag --andnot                                   match (`B' or `C') and no line in the file matches `D'.
# @flag -P --perl-regexp                           Interpret PATTERN as a Perl regular expression using PCRE2.
# @flag -p --no-dereference                        If -R or -r is specified, no symbolic links are followed, even when they are specified on the command line.
# @option --pager <COMMAND>                        When output is sent to the terminal, uses COMMAND to page through the output.
# @flag --pretty                                   When output is sent to a terminal, enables --color, --heading, -n, --sort and -T when not explicitly disabled or set.
# @option -Q --query <DELAY>                       Query mode: user interface to perform interactive searches.
...
# @flag -S --dereference                           If -r is specified, all symbolic links are followed, like -R.
# @flag -s --no-messages                           Silent mode: nonexistent and unreadable files are ignored, i.e.
# @option --save-config <FILE>                     Save configuration FILE.
...
# @flag -U --binary                                Disables Unicode matching for binary file matching, forcing PATTERN to match bytes, not Unicode characters.
...
# @option --view <COMMAND>                         Use COMMAND to view/edit a file in query mode when pressing CTRL-Y.
...
# @flag -y --any-line                              Any line is output (passthru).
# @option -Z --fuzzy <[+-~][MAX]>                  Fuzzy mode: report approximate pattern matches within MAX errors.
# @flag -z --decompress                            Decompress files to search, when compressed.
# @option --zmax <NUM>                             When used with option -z (--decompress), searches the contents of compressed files and archives stored within archives by up to NUM recursive expansions.
# @flag -0 --null                                  Prints a zero-byte (NUL) after the file name.
