...
# @flag --to                                     Copy the closure of paths from the local Nix store to the Nix store on machine.
# @flag --from                                   Copy the closure of paths from the Nix store on machine to the local Nix store.
# @flag --gzip                                   Enable compression of the SSH connection.
# @flag --include-outputs                        Also copy the outputs of [store derivation]s included in the closure.
# @flag -s --use-substitutes                     Attempt to download missing paths on the target machine using Nix’s substitute mechanism.
...
# @arg user-machine[`_module_ssh_host`] <user@machine>
