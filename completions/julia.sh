...
# @option --code-coveragenone <|user|all>       Count executions of source lines (omitting setting is equivalent to `user`)
# @option --code-coverage[`_choice_code_coverage`] <@<path>>  Count executions but only in files that fall under the given file path/directory.
# @option --track-allocationnone <|user|all>    Count bytes allocated by each source line (omitting setting is equivalent to `user`)
# @option --track-allocation <@<path>>          Count bytes but only in files that fall under the given file path/directory.
