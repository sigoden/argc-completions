...
# @option -f <directory>
# @option -g* <group_name> <device> <device>    group_name { device [...] | ALL } Display statistics for a group of devices.
...
# @option -j* <selector> <device> <device>      Display persistent device names.
...
# @option -p* <device> <device>                 Display statistics for block devices and all their partitions that are used by the system.
