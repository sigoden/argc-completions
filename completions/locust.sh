...
# @option --config-users* <CONFIG_USERS>      User configuration as a JSON string or file.
...
# @flag --web-login                           Protects the web interface with a login page.
...
# @flag --enable-rebalancing                  Re-distribute users if new workers are added or removed during a test run.
...
# @option -T --tags* <tag>                    List of tags to include in the test, so only tasks with at least one matching tag will be executed
