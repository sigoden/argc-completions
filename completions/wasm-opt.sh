...
# @flag -c --converge                            Run passes to convergence, continuing while binary size decreases
# @flag --fuzz-exec-before                       Execute functions before optimization, helping fuzzing find bugs
# @flag -feh                                     Execute functions before optimization, helping fuzzing find bugs
# @flag --fuzz-exec                              Execute functions before and after optimization, helping fuzzing find bugs
# @flag -fe                                      Execute functions before and after optimization, helping fuzzing find bugs
# @flag --extra-fuzz-command                     An extra command to run on the output before and after optimizing.
# @flag -efc                                     An extra command to run on the output before and after optimizing.
# @flag --translate-to-fuzz                      Translate the input into a valid wasm module *somehow*, useful for fuzzing
# @flag -ttf                                     Translate the input into a valid wasm module *somehow*, useful for fuzzing
# @flag --initial-fuzz                           Initial wasm content in translate-to-fuzz (-ttf) mode
# @flag -if                                      Initial wasm content in translate-to-fuzz (-ttf) mode
# @flag --fuzz-passes                            Pick a random set of passes to run, useful for fuzzing.
# @flag -fp                                      Pick a random set of passes to run, useful for fuzzing.
# @flag --no-fuzz-memory                         don't emit memory ops when fuzzing
# @flag --no-fuzz-oob                            don't emit out-of-bounds loads/stores/indirect calls when fuzzing
# @flag --emit-js-wrapper                        Emit a JavaScript wrapper file that can run the wasm with some test values, useful for fuzzing
# @flag -ejw                                     Emit a JavaScript wrapper file that can run the wasm with some test values, useful for fuzzing
# @flag --emit-wasm2c-wrapper                    Emit a C wrapper file that can run the wasm after it is compiled with wasm2c, useful for fuzzing
# @flag -esw                                     Emit a C wrapper file that can run the wasm after it is compiled with wasm2c, useful for fuzzing
# @flag --input-source-map                       Consume source map from the specified file
# @flag -ism                                     Consume source map from the specified file
...
# @flag --new-wat-parser                         Use the experimental new WAT parser
