...
# @flag --wasm-debug                             Include debug information in Wasm output.
# @flag -v*                                      Verbosity (none: info, errors & warnings, -v: verbose, -vv: very verbose)
# @option --js-minify[true|false] <JS_MINIFY>    Minify javascript assets with swc.
...
# {{ cargo leptos new
# @cmd Start a wizard for creating a new project (using cargo-generate)
