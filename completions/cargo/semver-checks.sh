...
# @option --target[`_choice_target`] <BUILD_TARGET>  Which target to build the crate for, to check platform-specific APIs, e.g. `x86_64-unknown-linux-gnu`
# @flag -v --verbose*                          Increase logging verbosity
# @flag -q --quiet*                            Decrease logging verbosity
# @option --color[auto|always|never] <WHEN>    Choose whether to output colors
