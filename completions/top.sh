...
# @option -p*,[`_choice_pid`] <mode>     Monitor only processes with specified process IDs.
...
# @option -u[`_choice_user`] <user>      Display only  processes  with a user id or user name matching that given.
# @option -U[`_choice_user`] <user>      Display only  processes  with a user id or user name matching that given.
