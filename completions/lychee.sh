...
# @flag --dump-inputs                            Don't perform any link extraction and checking.
...
# @option -s --scheme                            Only test links with the given schemes (e.g. https).
...
# @flag --exclude-mail                           Exclude all mail addresses from checking (deprecated; excluded by default)
# @flag --include-mail                           Also check email addresses
# @option --remap                                Remap URI matching pattern to different URI
# @option --header                               Custom request header
# @option -a --accept                            A List of accepted status codes for valid links
# @flag --include-fragments                      Enable the checking of fragments in links
...
# @option --cookie-jar <COOKIE_JAR>              Tell lychee to read cookies from the given file.
